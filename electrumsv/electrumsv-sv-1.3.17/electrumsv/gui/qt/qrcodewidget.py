...
class QRCodeWidget(QWidget):
    ...
    def paintEvent(self, e):
        ...
        qp.setBrush(white)
        qp.setPen(white)
        qp.drawRect(int(left-margin), int(top-margin), int(size+(margin*2)), int(size+(margin*2)))
        qp.setBrush(black)
        qp.setPen(black)
        ...
        for r in range(k):
            for c in range(k):
                if matrix[r][c]:
                    qp.drawRect(int(left + c * boxsize), int(top + r * boxsize), int(boxsize - 1), int(boxsize - 1))
        qp.end()
